...
func main() {
	...
	places, err := fetchAll(ctx, cfg)
	if err != nil {
		// 리스트 하나라도 실패하면 전체 중단 — 부분 데이터로 아카이브가 오작동하는 것을 막는다.
		log.Fatalf("네이버 조회 실패, 동기화 중단: %v", err)
	}
	log.Printf("네이버: 리스트 %d개에서 장소 %d곳 (SID 기준 병합)", len(cfg.Lists), len(places))
	...
	for _, sid := range sortedKeys(places) {
		m := places[sid]
		...
		case !exists:
			creates++
			if *dryRun {
				logSample("생성", creates, m.bm.Title(), m.lists)
				continue
			}
			if err := nc.CreatePage(ctx, placeProperties(m, false)); err != nil {
				log.Printf("생성 실패 %s(%s): %v", m.bm.Title(), sid, err)
			}
		case needsUpdate(page, m):
			updates++
			if *dryRun {
				logSample("갱신", updates, m.bm.Title(), m.lists)
				continue
			}
			// 수동으로 체크한 가봤음은 유지
			...
			if err := nc.UpdatePage(ctx, page.ID, props); err != nil {
				log.Printf("갱신 실패 %s(%s): %v", m.bm.Title(), sid, err)
			}
		default:
			unchanged++
		}
...
func fetchAll(ctx context.Context, cfg config.Config) (map[string]*mergedPlace, error) {
	client := naver.NewClient()
	places := map[string]*mergedPlace{}
	for _, entry := range cfg.Lists {
		...
		for _, b := range share.BookmarkList {
			...
			m.lists = append(m.lists, entry.Name)
			m.visited = m.visited || entry.Visited
			if m.bm.Memo == "" && b.Memo != "" {
				m.bm.Memo = b.Memo
			}
		}
...
func needsUpdate(page notion.Page, m *mergedPlace) bool {
	if page.Name != m.bm.Title() || page.Memo != m.bm.Memo {
		return true
	}
...
func placeProperties(m *mergedPlace, visited bool) map[string]any {
	b := m.bm
...
func sortedKeys(m map[string]*mergedPlace) []string {
